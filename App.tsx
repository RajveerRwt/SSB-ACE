...
const App: React.FC = () => {
  ...
  const navigateTo = async (test: TestType) => {
     ...
     // Enforce login for core tests (Consistency with Dashboard)
     if (!user && (
  ...
  const handleConsumeCoins = async (cost: number) => {
    if (!user) return false;
    const success = await deductCoins(user, cost);
    if (!success) {
        alert("Insufficient Coins. Please top up your account to continue.");
        setPaymentOpen(true);
    }
    return success;
  };
