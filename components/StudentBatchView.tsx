...
import { 
  ...
  Lock, ShieldCheck, Zap, RefreshCw, XCircle
} from 'lucide-react';
...
const StudentBatchView: React.FC<StudentBatchViewProps> = ({ userId, onStartTest }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // If a batch is selected, refresh its tests too
      if (selectedBatch) {
        const tests = await getBatchTests(selectedBatch.id);
        setBatchTests(tests);
      }
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-3">
          <button 
            onClick={fetchData}
            disabled={loading}
            className="p-4 bg-white border border-slate-200 text-slate-600 rounded-2xl hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center"
            title="Refresh Data"
          >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded ${
                          b.status === 'ACCEPTED' ? (selectedBatch?.id === b.batches.id ? 'bg-white/20 text-white' : 'bg-emerald-100 text-emerald-600') : 
          ...
          {selectedBatch ? (
            (() => {
              ...
              if (membership?.status !== 'ACCEPTED') {
                return (
                  <div className="bg-white rounded-[2.5rem] p-20 border border-slate-100 shadow-sm text-center flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-500">
                    <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 ${membership?.status === 'REJECTED' ? 'bg-red-50 text-red-500' : 'bg-amber-50 text-amber-500'}`}>
                      {membership?.status === 'REJECTED' ? <XCircle size={40} /> : <Clock size={40} />}
                    </div>
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter mb-2">
                      {membership?.status === 'REJECTED' ? 'Access Denied' : 'Approval Pending'}
                    </h3>
                    <p className="text-slate-400 text-sm font-medium italic max-w-xs">
                      {membership?.status === 'REJECTED' 
                        ? 'Your request to join this tactical unit has been rejected by the mentor. Contact HQ for further instructions.' 
                        : 'Your request to join this squad is awaiting mentor verification. You will be granted access to assessments once approved.'}
                    </p>
                  </div>
                );
              }
