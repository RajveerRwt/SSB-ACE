...
const MentorDashboard: React.FC<MentorDashboardProps> = ({ userId, userEmail, userName }) => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (selectedBatch) {
        const updatedBatch = mentorBatches.find(b => b.id === selectedBatch.id);
        if (updatedBatch) {
          setSelectedBatch(updatedBatch);
          setMeetingLink(updatedBatch.meeting_link || '');
          const [members, tests] = await Promise.all([
            getBatchMembers(updatedBatch.id),
            getBatchTests(updatedBatch.id)
          ]);
  ...
  const handleUpdateMeetingLink = async () => {
    ...
    try {
      await updateBatchMeetingLink(selectedBatch.id, meetingLink || null);
      const updatedBatches = await getMentorBatches(userId);
  ...
  const handleSelectBatch = async (batch: any) => {
    ...
    setMeetingLink(batch.meeting_link || '');
    setLoading(true);
    try {
      const [members, tests] = await Promise.all([
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  disabled={refreshing}
                  className={`w-10 h-10 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-100 transition-all flex items-center justify-center ${refreshing ? 'animate-spin' : ''}`}
                  title="Refresh Data"
                >
                  <RefreshCw size={18} />
                </button>
                <button 
                  ...
                  className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all duration-500 flex items-center justify-center shadow-lg shadow-blue-600/5 relative z-20"
                >
                  <Plus size={24} />
                </button>
              </div>
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-100">
                        <input 
                          ...
                          className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none w-64"
                        />
                        <button 
                        ...
                        >
                          {updatingLink ? <Loader2 size={16} className="animate-spin" /> : <Link size={16} />}
                        </button>
                      </div>
