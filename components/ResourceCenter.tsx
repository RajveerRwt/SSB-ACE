...
import { BookOpen, FileText, ImageIcon, Globe, Zap, PenTool, Layout, CheckCircle, Search, Calendar, Tag, MessageSquare, Loader2, ArrowRight, Mic } from 'lucide-react';
import { fetchDailyNews } from '../services/geminiService';
import { STANDARD_WAT_SET } from '../services/geminiService';

const ResourceCenter: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'WAT' | 'TAT' | 'LECTURETTE' | 'BLOG'>('WAT');
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden flex flex-col items-center text-center border-b-8 border-blue-500">
         <div className="relative z-10 space-y-4">
            ...
            <p className="text-slate-400 max-w-2xl font-medium leading-relaxed text-sm md:text-base">
               Free curated study material for SSB Interview preparation. Access WAT words, TAT samples, Lecturette topics, and daily defense current affairs.
            </p>
         </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-4 border-b border-slate-100 pb-6">
                      <div>
                          ...
                          <p className="text-slate-500 text-xs font-bold mt-2">3 Minutes to speak. Choose one topic and structure your thoughts.</p>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {LECTURETTE_TOPICS.map((topic, i) => (
                          <div key={i} className="flex items-center justify-between p-5 bg-white border border-slate-200 rounded-2xl hover:border-slate-400 transition-all group shadow-sm hover:shadow-lg">
                              <div className="flex items-center gap-4">
                                  <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-colors font-black text-xs">
                                      {i + 1}
                                  </div>
                                  <div>
