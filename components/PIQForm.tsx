...
const PIQForm: React.FC<{ onSave: (data: PIQData) => void, initialData?: PIQData }> = ({ onSave, initialData }) => {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 1: return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4">
          <div className="md:col-span-2">
            <InputField label="Name in CAPITALS" value={data.name} onChange={(v: string) => setData({...data, name: v.toUpperCase()})} />
          </div>
          <div className="md:col-span-2">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
          ...
          <div className="space-y-4">
            {data.family.map((member, i) => (
              <div key={i} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4 relative group">
                {i > 1 && (
                  <button onClick={() => removeFamilyMember(i)} className="absolute -top-2 -right-2 p-2 bg-red-100 text-red-600 rounded-full md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <Trash2 size={12} />
                  </button>
                )}
      ...
      case 4: return (
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
          <h4 className="text-sm font-black uppercase text-slate-600 tracking-widest">Educational Record</h4>
          <div className="space-y-6">
            {data.education.map((edu, i) => (
              <div key={i} className="p-6 md:p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-4">
      ...
      case 6: return (
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
          <div className="flex justify-between items-center">
            <h4 className="text-sm font-black uppercase text-slate-600 tracking-widest">Details of Previous Interviews</h4>
          ...
          {data.previousAttempts.length === 0 ? (
            ...
            <div className="space-y-4">
              {data.previousAttempts.map((attempt, i) => (
                <div key={i} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4 relative group">
                  <button onClick={() => removeAttempt(i)} className="absolute -top-2 -right-2 p-2 bg-red-100 text-red-600 rounded-full md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <Trash2 size={12} />
                  </button>
                  ...
                  <InputField label="Result" value={attempt.result} onChange={(v: string) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 md:space-y-8 pb-12">
      <div className="bg-slate-900 rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center relative z-10 gap-6 md:gap-8 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <button 
              onClick={() => onSave(data)}
              className="px-6 md:px-8 py-3 md:py-4 bg-yellow-400 text-black rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-3 shadow-xl hover:-translate-y-1 transition-all"
            >
              <Save size={18} /> Save
            </button>
          </div>
      ...
      <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 custom-scrollbar">
        {steps.map((s, i) => (
          <button 
            key={i} 
            onClick={() => setStep(i)}
            className={`flex items-center gap-2 md:gap-4 px-5 md:px-8 py-4 md:py-5 rounded-3xl border-2 transition-all shrink-0 font-black uppercase text-[10px] tracking-widest whitespace-nowrap ${step === i ? 'bg-slate-900 border-slate-900 text-white shadow-xl' : 'bg-white border-slate-100 text-slate-400 hover:border-slate-200'}`}
          >
      ...
      <div className="bg-white p-6 md:p-16 rounded-[2rem] md:rounded-[4rem] shadow-2xl border border-slate-100 relative">
        {isProcessing && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-md z-50 flex flex-col items-center justify-center rounded-[2rem] md:rounded-[4rem]">
             <div className="w-16 h-16 md:w-20 md:h-20 border-8 border-slate-100 border-t-slate-900 rounded-full animate-spin mb-8" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        <div className="p-8 md:p-10 bg-blue-50 rounded-[2rem] md:rounded-[3rem] border border-blue-100">
        ...
        <div className="p-8 md:p-10 bg-green-50 rounded-[2rem] md:rounded-[3rem] border border-green-100">
        ...
        <div className="p-8 md:p-10 bg-red-50 rounded-[2rem] md:rounded-[3rem] border border-red-100">
