    ...
    <div className="space-y-6 md:space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid lg:grid-cols-12 gap-6 md:gap-10">
         ...
         <div className="lg:col-span-4 space-y-6 md:space-y-10">
            <div className="bg-slate-950 p-8 md:p-12 rounded-[2rem] md:rounded-[4rem] text-white shadow-2xl relative overflow-hidden flex flex-col items-center text-center h-fit">
...
const App: React.FC = () => {
  ...
  const navigateTo = async (test: TestType) => {
     ...
     if ((test === TestType.INTERVIEW || test === TestType.PPDT || test === TestType.TAT) && user) {
        const { allowed, message } = await checkLimit(user, test.toString().toUpperCase());
        if (!allowed) {
            alert(message);
            setPaymentOpen(true);
            return;
  ...
  const renderContent = () => {
    switch (activeTest) {
      ...
      case TestType.INTERVIEW:
        return <Interview piqData={piqData || undefined} onSave={handleTestComplete} isAdmin={isUserAdmin(userEmail)} userId={user || undefined} />;
      ...
      case TestType.AI_BOT:
        return <SSBBot />;
      case TestType.ADMIN:
        return isUserAdmin(userEmail) ? <AdminPanel /> : null;
      case TestType.TERMS:
