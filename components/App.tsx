...
import { 
  getUserData, saveUserData, saveTestAttempt, getUserHistory, 
  checkAuthSession, syncUserProfile, subscribeToAuthChanges, 
  isUserAdmin, checkLimit, getUserSubscription, 
  getLatestPaymentRequest, incrementUsage, logoutUser, deductCoins 
} from '../services/supabaseService';
import { ShieldCheck, Quote, Zap, Star, Shield, Flag, ChevronRight, LogIn, Loader2, CheckCircle, Lock, Crown, History, AlertCircle, Clock } from 'lucide-react';
                  ...
                  >
                    <div className="flex items-center gap-4 md:gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-[9px] font-bold uppercase mt-1 tracking-widest">{test.type} • {test.time}</p>
                      </div>
                    </div>
...
const App: React.FC = () => {
  ...
  const navigateTo = async (test: TestType, params?: any) => {
     ...
     if ((test === TestType.INTERVIEW || test === TestType.PPDT || test === TestType.TAT) && user) {
        const { allowed, message } = await checkLimit(user, test.toString());
        if (!allowed) {
            alert(message);
            setPaymentOpen(true);
            return;
     ...
     if (params && params.cost && user) {
        const success = await handleCoinConsumption(params.cost);
        if(!success) return;
     }
  ...
  const handleTestComplete = async (result: any) => {
      ...
      // If using coins model, incrementing usage might be redundant if we already deducted coins,
      // but keeping it for stats tracking.
  ...
  const renderContent = () => {
    switch (activeTest) {
      ...
      case TestType.INTERVIEW:
        return <Interview piqData={piqData || undefined} onSave={handleTestComplete} isAdmin={isUserAdmin(userEmail)} userId={user || undefined} isGuest={!user} onLoginRedirect={() => setActiveTest(TestType.LOGIN)} onConsumeCoins={handleCoinConsumption} />;
      ...
      case TestType.AI_BOT:
        return <SSBBot />;
      case TestType.ADMIN:
        return isUserAdmin(userEmail) ? <AdminPanel /> : <Dashboard onStartTest={navigateTo} piqLoaded={!!piqData} isLoggedIn={!!user} isLoading={isLoading} user={userEmail || ''} onOpenPayment={() => setPaymentOpen(true)} subscription={subscription} onShowGuestWarning={handleShowGuestWarning} />;
      case TestType.TERMS:
      ...
      default: return <Dashboard onStartTest={navigateTo} piqLoaded={!!piqData} isLoggedIn={!!user} isLoading={isLoading} user={userEmail || ''} onOpenPayment={() => setPaymentOpen(true)} subscription={subscription} onShowGuestWarning={handleShowGuestWarning} />;
    }
  };
