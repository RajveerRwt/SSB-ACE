...
-- Handle migration for batch_members status
do $$
begin
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'batch_members' and column_name = 'status') then
    alter table public.batch_members add column status text default 'PENDING';
  end if;
end $$;
...
returns boolean as $$
begin
  return exists (select 1 from public.batch_members where batch_id = b_id and user_id = auth.uid() and status = 'ACCEPTED');
end;
$$ language plpgsql security definer;
