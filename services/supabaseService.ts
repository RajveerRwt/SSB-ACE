...
// --- DAILY CHALLENGE & GAMIFICATION (UPDATED) ---
...
// UPDATED: Now accepts single strings for wat/srt/interview, wraps them in arrays for DB consistency if needed
export const uploadDailyChallenge = async (ppdtFile: File | null, wat: string, srt: string, interview: string) => {
  ...
  // Storing as Arrays of length 1 for backward compatibility with schema, or updated schema
  // Note: ensure DB schema has interview_question column
  const { error: insertError } = await supabase.from('daily_challenges').insert({
      ppdt_image_url: ppdtUrl,
      wat_words: [wat], // Store as array
      srt_situations: [srt], // Store as array
      interview_question: interview // Store as text
  });
...
// UPDATED: Accepts single answers + interview answer
export const submitDailyEntry = async (challengeId: string, ppdt: string, wat: string, srt: string, interview: string) => {
  ...
  await supabase.from('daily_submissions').insert({
      ...
      user_id: user.id,
      ppdt_story: ppdt,
      wat_answers: [wat], // Store as array
      srt_answers: [srt], // Store as array
      interview_answer: interview // Store as text
  });
...
export const getDailySubmissions = async (challengeId: string) => {
  ...
  let { data, error } = await supabase
    ...
    .select(`*, aspirants(full_name, streak_count), submission_likes(user_id)`)
...
export const subscribeToAnnouncements = (callback: (announcement: Announcement) => void) => {
